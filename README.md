...
- **Quality (committed snapshot).** Real model calls are too costly/noisy for CI and the
  ...
  alongside it. The headline "+X points vs the honest baseline" claim lives in
  **[`RESULTS.md`](RESULTS.md)** and is code-generated by
  `metrics.py --headline` so it can't drift from the data. *(Status: pending the first
  real measurement — no quality lift is claimed yet.)*
